...
class Exp_Long_Term_Forecast_ood_vlbm(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_ood_vlbm, self).__init__(args)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            ...
            for i, (batch_x, batch_y, ylabel) in enumerate(test_loader):
                batch_x = batch_x.float().to(self.device, non_blocking=True)
                batch_y = batch_y.float().to(self.device, non_blocking=True)
                ylabel = ylabel.to(self.device, non_blocking=True)
                out = self.model(batch_x, batch_y)
        ...
        print('ALL mse:{}, mae:{}, mape:{}'.format(mse, mae, mape))

        mae, mse, mape = evaluation(torch.from_numpy(trues[labels==0]),torch.from_numpy(preds[labels==0]))
        print('normal mse:{}, mae:{}, mape:{}'.format(mse, mae, mape))

        mae, mse, mape = evaluation(torch.from_numpy(trues[labels==1]),torch.from_numpy(preds[labels==1]))
        print('abnormal mse:{}, mae:{}, mape:{}'.format(mse, mae, mape))
        return
